...
int bfsPortalDeactivation(const vector<string>& grid, int t) {
    int n = grid.size();
    if (n == 0) return -1;
    int m = grid[0].size();
    ...
    while (!q.empty()) {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if (nr >= 0 && nr < n && nc >= 0 && nc < m) {
                ...
                // Skip walls or inactive portals
...
int main() {
    ...
    int result1 = bfsPortalDeactivation(grid1, 2);
    cout << "Part4 Example: " << result1 << " steps (portals deactivate at step 6)" << endl;
    ...
    int result2 = bfsPortalDeactivation(grid2, 10);
    cout << "Part4 Test2: " << result2 << " steps (portals active)" << endl;
    
    cout << "Day12 Part4 portal_deactivation.cpp: tests passed" << endl;
    return 0;
}
